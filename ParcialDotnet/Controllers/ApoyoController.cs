...
namespace WebPulsaciones.Controllers
{
    ...
    public class ApoyoController : ControllerBase
    {
        ...
        {
            //var per = _personaService.BuscarxIdentificacion();
            var apoyos = _apoyoService.ConsultarTodos().Select(p => new ApoyoViewModel(p, _personaService.BuscarxIdentificacion(p.Persona.Identificacion)));
            return apoyos;
        }
